...
- **文字工具**：选择"文字"后点击画布输入（中文/英文均可），栅格化为图片对象，可随图片一起缩放旋转
- **板端顶部栏（eMP-gba/eMP-video 同款）**：90% 宽 × 38 高、radius 5、bg #EEEEEE@90%，
  400ms ease-out 展开/收回动画；按钮 = 截图(蓝) / 清空(绿) / 绘图开关(蓝⇄红"Lock") / x 收起；
  收起后点按/下滑屏幕顶缘重新展开
- **反向绘图**：顶栏"Draw"进入绘图模式后，板端触摸直接作画（板屏即时渲染并向浏览器广播），
  再点一下回到锁定（触摸不落笔）
