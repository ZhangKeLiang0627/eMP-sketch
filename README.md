...
- 撤销 / 重做（Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y + 工具栏按钮），浏览器与板端按"笔"粒度同步
- 无限画布平移缩放 + 视口跟随板屏
- 板端软件渲染器：Bresenham + 圆形印章 + RGB565 alpha 合成（每像素每次渲染只混合一次），直接写帧缓冲
- HTTP 静态托管（板子自带网页）+ WebSocket（draw/undo/redo/viewport/clear）
